...
class MaxQueue(Heap):
    ...
    def exchange(self,i,j):
        ...
        A[i].ndx = j
        A[j].ndx = i
        super(MaxQueue, self).exchange(i,j)
...
class DijkVertex(Vertex):
    def __init__(self, name):
        super(DijkVertex, self).__init__(name)
        self.d = INFTY
        self.priority = None
    def __repr__(self):
        return "(%s %s %s)" % (self.name,self.n,self.d)
    def add(self, v, w):
        a = Weight(v, w)
        a.next = self.first
        self.first = a
